...
module SlackService
  ...
  def post_hangout_notification(hangout, client = Slack::Web::Client.new(logger: Rails.logger))
    ...
    if hangout.category == "Scrum"
      send_slack_message client, CHANNELS[:general], "@here #{hangout.title}: #{hangout.hangout_url}", hangout.user
      send_slack_message client, CHANNELS[:standup_notifications], "@channel #{hangout.title}: #{hangout.hangout_url}", hangout.user
    ...
    if channel
      send_slack_message client, channel, "@here #{hangout.title}: #{hangout.hangout_url}", hangout.user
    end
  end
  ...
  def send_gitter_message_avoid_repeats(text)
    ...
    client.send_message(text, GITTER_ROOMS['saasbook/MOOC'])
  end
