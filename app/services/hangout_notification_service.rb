...
class HangoutNotificationService
  def self.with(event_instance,
                ...
                new(event_instance, user, slack_client, gitter_client).send(:run)
  end
  
  private
  
  def initialize event_instance, user, slack_client, gitter_client
  ...
  def test_react_frontend_hangout_notification channels
    send_slack_message @slack_client, channels, @here_message
  end
