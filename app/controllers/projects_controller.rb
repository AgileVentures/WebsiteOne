...
#TODO YA Add controller specs for all the code
  ...
  def index
    @projects = Project.where(status: 'Active').order('title ASC').order('last_github_update DESC NULLS LAST').order('commit_count DESC NULLS LAST').search(params[:search], params[:page]).includes(:user)
