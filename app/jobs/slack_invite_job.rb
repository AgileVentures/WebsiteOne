...
class SlackInviteJob
  ...
  def perform(email, token = Slack.config.token)
    ...
    unless json_response.present? && json_response['ok']
      AdminMailer.failed_to_invite_user_to_slack(email, error.try(:to_json),
                                                 slack_error_message.try(:to_json)).deliver_later
    end
  end
end
