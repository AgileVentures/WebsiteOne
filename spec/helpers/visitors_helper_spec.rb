...
describe VisitorsHelper do
  describe '#display_countdown' do
    ...
    it 'should display countdown based on input' do
      ...
      @event = stub_model(Event, next_occurrence_time: double(IceCube::Occurrence, to_datetime: DateTime.parse('2014-03-07 10:30:00 UTC')))
      expect(helper.display_countdown(@event)).to eq '2 days 1 hours 15 minutes'
    end
    it 'should display countdown based on input and remove days when 0' do
      ...
      @event = stub_model(Event, next_occurrence_time: double(IceCube::Occurrence, to_datetime:DateTime.parse('2014-03-07 10:30:00 UTC')))
      expect(helper.display_countdown(@event)).to eq '1 hours 15 minutes'
    end
  end
